...
def doLsServer(controlSock):
	...
	dataSocketListen = sendrecv.getEphSocket()

	ephPort = dataSocketListen.getsockname()[1] #get port to which it is bound
	
	print "Using ephemeral port: ", ephPort
		
	sendrecv.sendMsg(controlSock, str(ephPort))
	
	print "Waiting for client data connection"
	# Accept the connection
	(dataSock, addr) = dataSocketListen.accept();
	...
	filedBuffer = ', '.join(i for i in files);

	#sendDataString(dataSock, filedBuffer);
	
	sendrecv.sendMsg(dataSock, filedBuffer)
...
while 1:
	...
	while command != "quit":
		...
		if command.startswith("put"):
			...
			dataSock = sendrecv.getEphSocket()
			
			# Get the ephemeral port
			ephPort = dataSock.getsockname()[1]
			
			# Send the port to the client
			ephPortStr = str(ephPort)
			
			# Send the port to the client
			sendrecv.sendMsg(controlSocket, ephPortStr)
			
			# Wait for the client to connect	
			dataCommSock, addr = dataSock.accept()	
					
			sendrecv.recvFile("recv_" + fileName, dataCommSock)
			dataSock.close()
			...
			# get the ephemeral socket 
			dataSock = sendrecv.getEphSocket()

			#get the ephemeral port name
			ephPort = dataSock.getsockname()[1]

			ephPortStr = str(ephPort) # convert to string

			# send the port to the client in string format
			sendrecv.sendMsg(controlSocket, ephPortStr)
			
			print "Waiting for the client to connect..."
	
			# wait fo rthe client to connect
			dataCommSock, addr = dataSock.accept()
			
			print "Client connected ", addr
			
			print "Sending file..."
			sendrecv.sendFile(fileName, dataCommSock)
			print "Done.."
			dataSock.close()
