...
#sendrecv.sendFile("testFile.txt", sock)
...
while 1:
	...
	if command.startswith("put"):
		# send a message to the server with the command using the control Socket
		sendrecv.sendMsg(controlSocket, command)
		# receive string essage from the server using the control socket
		ephPortNum = sendrecv.recvMsg(controlSocket)
		# create the socket for the ephemeral 
		ephSocket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
		# connect the ephSocket with serverName and ephemeral port num (int)
		ephSocket.connect((serverName, int(ephPortNum)))
		# send the file Name to the server using the ephemeral socket
		textFile = command.split(' ')[1]
		
		sendrecv.sendFile(textFile, ephSocket)
		
		# closing the control socket closes the ephSocket
		ephSocket.close()

	elif command.startswith("get"):
		# send the message with the get command to the server
		sendrecv.sendMsg(controlSocket, command)
	
		#receive stirnng message form the server using the control socket
		ephPortNum = sendrecv.recvMsg(controlSocket)
		# creat the ephemeral socket to send the text file
		ephSocket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
		# connect the ephemeralSocket to server
		ephSocket.connect((serverName, int(ephPortNum)))
		#receive the file to the server
		textFile = command.split(' ')[1]
		sendrecv.recvFile(textFile, ephSocket)
		# closing the control socket closes the ephSocket
		ephSocket.close()

	elif command == "ls":
		# send ls message to the server
		sendrecv.sendMsg(controlSocket, "ls")
		
		# receive message of the ephemeral port number from control socket
		ephPortNum = sendrecv.recvMsg(controlSocket)
		
		# make a connection socket to ephPortNum
		dataSock = connectSocket(serverName, int(ephPortNum))
		# receive the file list in the server from the dat socket
		fileList = sendrecv.recvMsg(dataSock)
		print fileList
		dataSock.close()
